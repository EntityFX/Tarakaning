<?php
	class ItemsFacade
	{
		...
		public function addItem(ItemDBKindENUM $kind, ErrorPriorityENUM $priority, ErrorTypeEnum $type, $title, $hoursRequired, $description="", $steps="", $assignedTo=null)
		{
			$itemID=(int)$this->_itemsController->addReport($kind, $priority, $type, $title, $hoursRequired, $description, $steps, $assignedTo);
			$this->_historyController->addHistory($this->_userID,$itemID,self::ADD_ITEM);
			return $itemID;
		}

		public function editItem($reportID,ErrorStatusENUM $newStatus, $newBugsData)
		{
			...
			$res=$this->_itemsController->editItem(
				$reportID,
				$this->_userID,
				$this->_projectID,
				$newBugsData["Title"],
				$newBugsData["HoursRequired"],
				$newBugsData["AddHours"],
				$newStatus,
